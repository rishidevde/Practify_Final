...
class InterviewRequest(SQLModel, table=True):
    """Interview connection request from a user"""
    id: int | None = Field(default=None, primary_key=True)
    created_at: datetime = Field(default_factory=datetime.utcnow, index=True)
    
    requester_id: int = Field(index=True, foreign_key="user.id")
    exam_type: ExamType = Field(index=True)
    status: str = Field(default="waiting")  # waiting | matched | expired
    session_id: int | None = Field(default=None, foreign_key="interviewsession.id")
...
class InterviewSession(SQLModel, table=True):
    ...
    initiator_id: int = Field(index=True, foreign_key="user.id")
    partner_id: int = Field(index=True, foreign_key="user.id")
    exam_type: ExamType = Field(index=True)
    
    status: str = Field(default="active")  # active | ended
    room_id: str = Field(unique=True)  # WebRTC room ID
